...
> **Inside a spec-loop run:** the "ask your human partner" exceptions above are a human gate governed by `spec-loop:escalation-gate` — do not stop the slice to ask. A slice worker takes the exception only when the plan explicitly scoped the work as a throwaway prototype, generated code, or config; otherwise TDD applies as written and any doubt is logged, not surfaced. Outside a run (interactive use), ask your human partner directly. The verification gates below (Verify RED / Verify GREEN) are never a human gate and are never skipped.

## When NOT to use this

- **You have a bug or unexpected behavior and don't yet know the cause** → start with `spec-loop:systematic-debugging` to find root cause, then come back here to write the failing test that reproduces it before you fix it.
...
**MANDATORY. Never skip.** This is not a human gate — run it yourself, every time, inside a run or not.
...
Don't add features, refactor other code, or "improve" beyond the test. YAGNI ("You Aren't Gonna Need It") — build for the test in front of you, not the feature you imagine coming next.

### Verify GREEN - Watch It Pass

**MANDATORY.** Not a human gate — run it yourself.
...
**Never fix bugs without a test.** When a bug's cause is not yet obvious, find root cause first with `spec-loop:systematic-debugging`, then return here to write the reproducing test before touching the fix.
