...
const SPEC_LOOP_VERSION = readSpecLoopVersion();
...
function readSpecLoopVersion() {
  ...
  for (const manifest of manifests) {
    try {
      ...
      // Manifest absent or malformed; try the next candidate.
    }
  }
...
// Plain-text branding only — no external image fetch, no remote host. Keeps the
// companion self-contained (no network egress) and free of third-party assets.
function brandMarkup() {
  const version = escapeHtmlText(SPEC_LOOP_VERSION);
  const text = 'Spec-Loop Brainstorm v' + version;
  return '<div class="brand"><a href="https://github.com/z2297/spec-loop"><span class="brand-copy">' + text + '</span></a></div>';
}
